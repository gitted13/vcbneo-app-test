...
export default function NapasCore() {
  ...
  /* NAPAS xác nhận TC (tiền đã về) nhưng Core chưa ghi có — chỉ áp dụng
     chiều Đến (tiền KH tự chuyển đi mà Core chưa trừ không phải case cần
     "hoàn trả", ngược nghĩa — đã bỏ khỏi báo cáo này). Yêu cầu khách hàng. */
  const handleExportRefundReport = async () => {
    const pending = rows.filter(r => {
      if (!r.napas || r.napas.failed || r.core || r.direction !== 'Đến') return false
      const d = r.napas.date
      if (filterFrom && d && dayToISO(d) < filterFrom) return false
      if (filterTo   && d && dayToISO(d) > filterTo)   return false
      return true
    })
    const headers = ['Ngày NAPAS', 'Trace', 'Sequence', 'Số tiền (VNĐ)', 'Loại (GD/QT)', 'Hành động']
    const rowsOut = pending.map(r => [
      r.napas.date, r.trace, r.sequence ?? '', r.amount, r.napas.type, 'Cần báo có cho KH',
    ])
    const date = new Date().toLocaleDateString('vi-VN').replace(/\//g, '-')
    await downloadDetailXlsx({
      title: 'GD NAPAS TC ĐẾN – CHƯA GHI CÓ Ở CORE (CẦN BÁO CÓ KH)',
      dir: 'Đến', filterFrom, filterTo, headers, rows: rowsOut,
      headerBg: '#b91c1c', filename: `NAPAS_CanBaoCoKH_${date}`,
    })
  }
    ...
    >
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.cardBorder}`, borderRadius: radius.lg, boxShadow: shadow.sm, overflow: 'hidden' }}>
        <div style={{ display: 'flex', gap: 10, padding: '12px 16px', borderBottom: `1px solid ${C.cardBorder}`, background: C.neutralBg, flexWrap: 'wrap' }}>
          ...
          <Button size="sm" variant="subtle" onClick={handleExportRefundReport} title="NAPAS Đến đã báo TC nhưng Core chưa ghi có — không phụ thuộc tab Đi/Đến đang xem, luôn xuất chiều Đến">↓ GD cần báo có KH</Button>
        </div>
