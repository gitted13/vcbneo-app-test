"""
extract_napas_den.py – Đọc "Napas đến.xlsx" → insert vào napas_raw (direction='Den', failed=0).

Dùng:
    python extract_napas_den.py <file.xlsx> [--year 2026]

Cấu trúc file NAPAS Đến:
  - Mỗi sheet = 1 ngày xử lý
  - Cột: STT | ... | Số tiền | Số trace | Giờ GD (HHMMSS) | Ngày GD (MMDD)
  - Vị trí cột mặc định: 0=STT, 2=Số tiền, 3=Số trace, 4=Giờ GD, 5=Ngày GD
"""
import sys, io
...
if len(sys.argv) < 2:
    print('Dùng: python extract_napas_den.py <file.xlsx> [--year YYYY]', file=sys.stderr)
    sys.exit(1)
...
for sh_name in wb.sheetnames:
    ...
    if c_trace is None or c_amt is None:
        # fallback: 0=STT, 2=Số tiền, 3=Số trace, 4=Giờ, 5=Ngày GD
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        rows_out.append((p, 'Den', trace, txn_date, ntime, amt, 0, typ))
        count += 1

    print(f'  Sheet "{sh_name}": {count} dòng.', file=sys.stderr)

wb.close()
print(f'Tổng: {len(rows_out)} dòng NAPAS Đến.', file=sys.stderr)
